...
import { pruneCommunities, scoreCommunity } from "./community-prune"
import type { AtomQualityMetrics, RankedAtom, TraversedAtom } from "./types"
...
export async function scoreAtomQuality(atoms: TraversedAtom[]): Promise<Record<string, AtomQualityMetrics>> {
  ...
  const pruned = cache ? await pruneCommunities() : undefined
  const decisions = new Map(pruned?.decisions.map((item) => [item.community.id, item]) || [])
  ...
  return Object.fromEntries(
    atoms.map((item) => {
      ...
      const decision = communityId ? decisions.get(communityId) : undefined
      let communityScore = decision ? scoreCommunity(decision) / 100 : 0.5
      if (decision?.pruned) {
        communityScore *= 0.5
      }
