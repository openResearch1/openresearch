...
export const SimpleTestWorkflowTemplate = WorkflowSchema.Template.parse({
  ...
  defs: {
    ...
    spawn_wait: {
      kind: "spawn_wait",
      title: "Spawn async agent and wait",
      summary: "Spawn a peer agent, yield the workflow turn, then consume the child_done result.",
      prompt: "spawn-wait",
      policy: {
        can_next: ["child_summary != null"],
        can_wait_interaction: false,
        can_edit_future: false,
        allowed_edit_ops: [],
      },
    ...
    spawn_finish: {
      kind: "spawn_finish",
      title: "Finish spawn wait test",
      summary: "Complete the dedicated spawn_agent workflow validation flow.",
      prompt: "spawn-finish",
      policy: {
        can_next: [],
        can_wait_interaction: false,
    ...
    parent_wait_child: {
      kind: "parent_wait_child",
      title: "Spawn child that waits for parent input",
      summary: "Spawn a child workflow that calls wait_interaction, resume it, then consume child_done.",
      prompt: "parent-wait-child",
      policy: {
        can_next: ["child_summary != null"],
        can_wait_interaction: false,
        can_edit_future: false,
    ...
    parent_wait_finish: {
      kind: "parent_wait_finish",
      title: "Finish parent wait test",
      summary: "Complete the parent-side child_waiting validation flow.",
      prompt: "parent-wait-finish",
      policy: {
        can_next: [],
        can_wait_interaction: false,
        can_edit_future: false,
    ...
    child_wait_parent: {
      kind: "child_wait_parent",
      title: "Wait for parent input",
      summary: "Pause with wait_interaction so the parent agent must resume this child.",
      prompt: "child-wait-parent",
      policy: {
        can_next: ["parent_answer != null"],
        can_wait_interaction: true,
        can_edit_future: false,
        allowed_edit_ops: [],
      },
    ...
    child_wait_finish: {
      kind: "child_wait_finish",
      title: "Finish child wait test",
      summary: "Complete the child-side parent input validation flow.",
      prompt: "child-wait-finish",
      policy: {
        can_next: [],
        can_wait_interaction: false,
        can_edit_future: false,
  ...
  flows: {
    ...
    spawn_agent_wait: {
      title: "Spawn Agent Wait",
      summary: "spawn-agent-wait",
      steps: ["spawn_wait", "spawn_finish"],
    },
    spawn_agent_parent_wait: {
      title: "Spawn Agent Parent Wait",
      summary: "spawn-agent-parent-wait",
      steps: ["parent_wait_child", "parent_wait_finish"],
    },
    child_parent_interaction: {
      title: "Child Parent Interaction",
      summary: "child-parent-interaction",
      steps: ["child_wait_parent", "child_wait_finish"],
    },
  },
