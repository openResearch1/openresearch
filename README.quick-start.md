...
- Linux 和 macOS 通常需要先赋予执行权限再运行：`chmod +x ./可执行文件名 && ./可执行文件名`。其中 macOS 如果首次运行被系统拦截，除了执行权限外，通常还需要去掉下载文件的隔离属性，可执行 `xattr -cr ./可执行文件名` 后再运行；如果仍然被拦截，可在“系统设置 -> 隐私与安全性”中允许后重新打开。
